...
class PerspectiveTransform(object):
    ...
    def __call__(self, img, lbl):
        ...
        point1 = [randint(0, int(0.10*cols)), randint(0, int(0.10*rows))]
        point2 = [randint(int(0.9*cols), int(1.0*cols)), randint(0, int(0.10*rows))]
        point3 = [randint(0, int(0.10*cols)), randint(int(0.9*rows), int(1.0 * rows))]
        point4 = [randint(int(0.9*cols), int(1.0*cols)), randint(int(0.9*rows), int(1.0 * rows))]
        p2 = np.float32([point1, point2, point3, point4])

        M = cv2.getPerspectiveTransform(p1,p2)
        img = cv2.warpPerspective(img, M, (cols, rows), flags=cv2.INTER_LINEAR)
        lbl = cv2.warpPerspective(lbl, M, (cols, rows), flags=cv2.INTER_NEAREST)
...
class ExtRandomCrop(object):
    ...
    def __init__(self, size, padding=0, pad_if_needed=False):
        ...
        self.pad_if_needed = pad_if_needed # true
